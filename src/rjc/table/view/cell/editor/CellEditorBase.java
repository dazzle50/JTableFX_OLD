...
public class CellEditorBase
{
  ...
  public void open( Object value, CellStyle cell )
  {
    ...
    if ( m_control instanceof XTextField textField )
    {
      // set min & max width
      ...
      if ( min > max )
        min = max;
      textField.setPadding( cell.getZoomTextInsets() );
      textField.setFont( cell.getZoomFont() );
      textField.setWidths( min, max );
      textField.setStatus( cell.view.getStatus() );
    }
  ...
  public boolean commit()
  {
    ...
    int dataColumn = m_cell.view.getColumnsAxis().getDataIndex( m_cell.columnIndex );
    int dataRow = m_cell.view.getRowsAxis().getDataIndex( m_cell.rowIndex );

    // push new command on undo-stack to update cell value
    // TODO var command = new CommandSetValue( data, columnIndex, rowIndex, getValue() );
